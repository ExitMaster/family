...
const state = {app:null,auth:null,db:null,user:null,membership:null,family:null,allowlist:null,isAdmin:false,members:[],households:[],categories:[],transactions:[],settlements:[],shares:[],careEvents:[],chemoCycles:[],healthLogs:[],payoutAccounts:[],attachments:[],tab:'home',ledgerFilter:'all',modal:null,message:null,unsubs:[]};
...
function renderVerify(user,msg=''){
  $('#app').innerHTML=`<div class="auth"><div class="auth-card"><h1>이메일 확인</h1><p class="sub">${esc(user.email)}로 보낸 Firebase 인증 메일의 링크을 누른 뒤 아래 버튼을 선택하세요.</p>${msg?`<div class="notice success">${esc(msg)}</div>`:''}<button id="verified" class="btn" style="width:100%">인증 완료 확인</button><button id="resend" class="btn ghost" style="width:100%;margin-top:8px">인증 메일 다시 보내기</button><button id="logout" class="btn ghost" style="width:100%;margin-top:8px">다른 계정 사용</button></div></div>`;
...
async function loadFamily(){
  try{
    ...
    let fsnap=await getDoc(familyRef());if(!fsnap.exists()){if(!state.isAdmin)return renderUnauthorized('가족 공간 초기 설정이 아직 완료되지 않았습니다.');await setDoc(familyRef(),{name:'딸꿍또꿍아꿍',created_at:serverTimestamp(),updated_at:serverTimestamp()});await ensureDefaults();fsnap=await getDoc(familyRef());}
...
function renderUnauthorized(msg){$('#app').innerHTML=`<div class="auth"><div class="auth-card"><h1>가족 전용 공간</h1><p class="sub">현재 계정: ${esc(state.user?.email||'')}</p><div class="notice error">${esc(msg)}</div><p class="muted" style="font-size:12px;line-height:1.6">Firestore의 <b>access/allowlist</b> 문서에는 최대 8개의 승인 이메일만 둡니다.</p><button id="logout" class="btn ghost" style="width:100%">다른 계정으로 로그인</button></div></div>`;$('#logout').onclick=()=>signOut(state.auth);}
