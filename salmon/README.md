...
서버에 AI 키(`CLAUDE_API_KEY` 또는 `GEMINI_API_KEY`)가 없으면 앱은 자동으로 **수동 모드**로
동작한다. `/api/claude`의 `health` 액션으로 키 유무를 확인하고, 키가 없으면 AI 버튼
(정리하기·쪼개기·마일스톤 제안·아이디어 AI 제안·인박스 자동분류)을 숨기거나 비활성화한다.
덤프·정리·표시 시점 오버레이·수동 우선순위(고정)·프로젝트/마일스톤 수동 관리·포커스·
뽀모도로는 키 없이 전부 작동한다. AI 프로바이더 확정 후 서버 환경변수만 추가하면 기능이 켜진다.
